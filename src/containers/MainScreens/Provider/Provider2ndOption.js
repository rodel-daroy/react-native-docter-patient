...
export class Provider2ndOption extends React.Component{
    constructor(props){
        super(props);
    }
    ...
    componentDidMount() {
        ...
        this.props.getProviderAllSlots({ currentDate: day, serviceType: '2', datePo: this.state.selectedTab, providerTimeZone: this.props.user.tz })
    }
        ...
        if(prevProps.providerSlots !== this.props.providerSlots) {
            if(this.props.providerSlots) {
                ...
                if (this.state.selectedTab == '1'){
                    start_time = this.getTimeStamp(this.state.selectedDate + ' 08:00:00');
                    end_time = this.getTimeStamp(this.state.selectedDate + ' 11:59:59');
                } else {
                    start_time = this.getTimeStamp(this.state.selectedDate + ' 12:00:00');
                    end_time = this.getTimeStamp(this.state.selectedDate + ' 19:59:59');
                }
                ...
                for(var i = start_time; i < end_time; i = i + (15 * 60 * 1000)) {
                    if(this.getTimeStamp(this.getDateFormat(0)) <= i) {
                        ...
                        // obj.slotTime = new Date(i).toLocaleTimeString();
                        if(this.state.selectedTab == "1") {
    ...
    selectTab = (tabIndex) => {
        this.props.getProviderAllSlots({currentDate: this.state.selectedDate, serviceType: '2', datePo: tabIndex, providerTimeZone: this.props.user.tz})
    ...
    selectTime = (index) => {
        ...
        if( slotArray[index].isBooked == false) {
            ...
            this.props.setProviderScheduleSlots({currentDate: this.state.selectedDate, serviceType: '2', datePo: this.state.selectedTab, saveData: slotArray[index], providerTimeZone: this.props.user.tz})
    ...
    selectAll = (status) => {
        ...
        if(slotArray.length > 0) {
            ...
            // this.setState({allSlots: slotArray})
            this.props.setProviderSlotCheckAll({currentDate: this.state.selectedDate, serviceType: '2', datePo: this.state.selectedTab, saveData: slotArray, status: status, providerTimeZone: this.props.user.tz})
        }
    }
...
export default connect(mapStateToProps, mapDispatchToProps)(Provider2ndOption)
