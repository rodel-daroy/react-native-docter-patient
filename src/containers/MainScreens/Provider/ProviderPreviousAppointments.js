...
import { getProviderAppointments } from "../../../redux/actions/providerActions"
...
export class ProviderPreviousAppointments extends React.Component{
    constructor(props){
        super(props);
    }
    ...
    componentDidMount(){
        this.props.getProviderAppointments({status: "previous"});
    }

    componentDidUpdate(prevProps) {
        if(prevProps.appointmentList !== this.props.appointmentList) {
            if(this.props.appointmentList.providerAppointments) {
                var previousAppointmentArray = this.props.appointmentList.providerAppointments;
            ...
            <Container style={MainStyles.container}>
                ...
                <ScrollView>
                    ...
                    <ScrollView horizontal = {true} showsVerticalScrollIndicator = {true} alwaysBounceVertical = {true} style = {{height: LAYOUT.height}}>
                        <DataTable style = {{width: LAYOUT.window.width * 3.2, height: "auto", marginTop: 50}}>
                            ...
                            {this.state.pageArray ? this.state.pageArray.length > 0 ? this.state.pageArray.map((item, id) => {
                                return (
                                    <DataTable.Row key = {id + 'PreviousAppointments'} onPress = {() => this.toggleModal(item)}>
                    ...
                    <DataTable.Pagination
                        ...
                        label={`${this.state.fromPage}-${this.state.toPage} of ${this.state.previousAppointmentArray.length}`}
                    />
                </ScrollView>
                <Footer />
                <Modal
...
const mapDispatchToProps = {
    getProviderAppointments, getChatRoomAllData
}

export default connect(mapStateToProps, mapDispatchToProps)(ProviderPreviousAppointments)
