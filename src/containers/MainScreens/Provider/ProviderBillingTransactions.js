...
import { getBillingData } from "../../../redux/actions/providerActions"
...
export class BillingPayments extends React.Component{
    ...
    componentDidUpdate(prevProps) {
        ...
        if(prevProps.providerBillingTransiction !== this.props.providerBillingTransiction) {
            if(this.props.providerBillingTransiction.providerBillingTransiction){
                var total_datas = this.props.providerBillingTransiction.providerBillingTransiction;
                var datas = [];
                for(var i = 0; i < total_datas.length; i ++) {
                    if(total_datas[i].status != 0) {
                        datas.push(total_datas[i]);
                    }
                }
                
                for(var i = 0; i < datas.length; i ++) {
                    var a_data = datas[i];
                    datas[i].appointment_date = this.get_correct_time_with_time_zone(datas[i].appointment_date);
                    var fv = a_data.service_id == 1 ? "40.00" : "150.00";
                    datas[i].final_amount = fv;
                    if(a_data.coupon_code && a_data.coupon_code != "") {
                        var v = fv;
                        if(a_data.coupon_percentage < 100) {
                            v = fv * (100 - a_data.coupon_percentage) / 100;
                        } else if(a_data.coupon_percentage == 100) {
                            v = fv;
                        } else if(a_data.coupon_percentage == 200) {
                            v = a_data.final_amount;
                        } else {
                            
                        }
                        datas[i].final_amount = v;
                    }

                    if(datas[i].status == '2') {
                        if(datas[i].coupon_code == "" || datas[i].coupon_percentage != 200) {
                            if(datas[i].canceled_by == "Provider") {
                                datas[i].final_amount = parseFloat(0.00);
                            } else if(datas[i].canceled_by == "Member") {
                                datas[i].final_amount = parseFloat(10.00);
                ...
                for(var i = 0; i < datas.length; i ++) {
                    total_amount += parseFloat(datas[i].final_amount);
                }
                
                var real_amount = total_amount;
                // if (datas.length > itemsPerPage){
                //     var pageArr = [];
                //     real_amount = 0;
                //     for(var i = 0; i < itemsPerPage; i ++){
                //         pageArr.push(datas[i]);
                //         real_amount += parseFloat(datas[i].final_amount);
                //     }
                //     this.setState({fromPage: datas.length > 0 ? 1 : 0, toPage: itemsPerPage, searchArray: datas, pageArray: datas, pageData: pageArr, numberOfPages: Math.floor(datas / itemsPerPage), totalAmount: total_amount});
                // } else {
                    this.setState({fromPage: datas.length > 0 ? 1 : 0, toPage: datas.length, searchArray: datas, pageArray: datas, pageData: datas, numberOfPages: Math.floor(datas / itemsPerPage), totalAmount: total_amount});
                // }
    ...
    changeFromDate = async (date) => {
        ...
        await this.setState({fromDate: date, pageData: searchArray,  totalAmount: this.calculateAmount(searchArray), numberOfPages: Math.floor(searchArray / itemsPerPage)})
        // await this.setState({fromDate: date, searchArray: searchArray,  totalAmount: this.calculateAmount(searchArray), numberOfPages: Math.floor(searchArray / itemsPerPage)})
        // this.setPage(0, true);
    }
    ...
    changeToDate = async (date) => {
        ...
        var searchArray = [];
        for(var i = 0; i < this.state.pageArray.length; i ++) {
            if(this.state.pageArray[i].appointment_date <= date && this.state.pageArray[i].appointment_date >= this.state.fromDate || date == "") {
                searchArray.push(this.state.pageArray[i]);
            }
        }
        
        await this.setState({toDate: date, pageData: searchArray, totalAmount: this.calculateAmount(searchArray), numberOfPages: Math.floor(searchArray / itemsPerPage)})
        // await this.setState({toDate: date, searchArray: searchArray, totalAmount: this.calculateAmount(searchArray), numberOfPages: Math.floor(searchArray / itemsPerPage)})
        // this.setPage(0, true);
    }
    ...
    render(){
        return(
            <Container style={MainStyles.container}>
                <Header
                    headerText = {"Biling / Payments"}
                    backURL = {"blank"}
                    backType = {"pop"}
                />
                <View style={MainStyles.HeaderStyle}></View>
                <View style={{flexDirection: 'row', paddingLeft: 20, paddingRight: 20}}>
                    <View style = {{borderBottomColor: COLOR.mainColor, borderBottomWidth: 1, flex: 2}}>
                        <Picker
                        ...
                        ?this.state.pageData.map((item, key)=>(
                            <View key={key} style={{
                                ...
                                <View style={{width:'100%', flexDirection:'row',}}>
                                    <View style={{width:'30%'}}>
                                        <Text style={{marginTop:5, fontSize:18, fontWeight:'700'}}>{`${this.props.user.first_name} ${this.props.user.last_name}`}</Text>
                                        <Text style={{marginTop:5, fontSize:18, fontWeight:'700'}}>{`${item.patientFirstName} ${item.patientFirstName}`}</Text>
                                ...
                                {this.state.activeItem===key&&<View style={{marginBottom:10,}}>
                                    ...
                                    <View style={{marginTop:10, flexDirection:'row', justifyContent:'space-between', alignItems:'flex-start'}}>
                                        ...
                                        <Text style={{fontSize:18}}>{
                                            (() => {
                                                if(item.status == '2') {
                                                    if(item.coupon_code == "" || item.coupon_percentage != 200) {
...
const mapStateToProps = (state) => {
    return {
        ...
        providerBillingTransiction: state.provider
    }
}
